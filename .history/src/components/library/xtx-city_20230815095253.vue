  ...
  setup () {
    ...
    const onChangeItem = (item) => {
      ...
      if (item.level === 1) {
        ...
        changeResult.countryCode = ''
        changeResult.countryName = ''
        changeResult.fullLocation = item.name
        ...
        changeResult.countryCode = ''
        changeResult.countryName = ''
        changeResult.fullLocation = changeResult.provinceName + item.name
      } else if (item.level === 3) {
        // 点击的是区县
        changeResult.countryCode = item.code
        changeResult.countryName = item.name
        changeResult.fullLocation =
          changeResult.provinceName + changeResult.cityName + item.name
      }
      // 隐藏城市列表
      onHide()
      // 触发change事件
